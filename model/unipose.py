...
class unipose(nn.Module):
    ...
    def forward(self, input):
        ...
        kpts = []
        for arr_2d in x:
            # _y, _x = (arr_2d==arr_2d.max()).nonzero()[0]
            idx = arr_2d.argmax()
            _y = torch.div(idx, arr_2d.shape[0], rounding_mode="floor") / arr_2d.shape[0]
            _x = idx % arr_2d.shape[1] / arr_2d.shape[1]
            # _x = _x / arr_2d.shape[1]            
            # _y = _y / arr_2d.shape[0]
            kpts.append((_y, _x))

        return torch.tensor(kpts)
